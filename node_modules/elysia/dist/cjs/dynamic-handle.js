...
const createDynamicHandler = (app) => async (request) => {
    ...
    try {
        ...
        context.cookie = (0, cookie_1.parseCookie)(context.set, context.headers.cookie, cookieMeta
            ? {
        ...
        if (context.set.cookie && cookieMeta?.sign) {
            ...
            if (cookieMeta.sign === true)
                for (const [key, cookie] of Object.entries(context.set.cookie))
                    context.set.cookie[key].value = (0, cookie_signature_1.sign)(cookie.value, '${secret}');
            else
                for (const name of cookieMeta.sign) {
                    if (!(name in cookieMeta.properties))
                        continue;
                    if (context.set.cookie[name]?.value) {
                        context.set.cookie[name].value = (0, cookie_signature_1.sign)(context.set.cookie[name].value, secret);
                    }
                }
        }
